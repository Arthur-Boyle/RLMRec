...
class LightGCNWithWeightedSum(BaseModel):
    def __init__(self, data_handler):
        super(LightGCNWithWeightedSum, self).__init__(data_handler)
        self.adj = data_handler.torch_adj
        self.keep_rate = configs['model']['keep_rate'] # 使用 get 方法设置默认值
        ...
        # 可学习的层权重
        self.layer_weights = nn.Parameter(t.ones(self.layer_num + 1) / (self.layer_num + 1))
    ...
    def forward(self, adj=None, keep_rate=1.0):
        ...
        final_embeds = t.sum(final_embeds * self.layer_weights.unsqueeze(-1).unsqueeze(-1), dim=0)
